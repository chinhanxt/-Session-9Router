...
import { 
  List, FolderOpen, FileDown, Trash2, ShieldAlert, User, Cpu, Zap
} from 'lucide-react';
...
export function LibraryManager({
  ...
  const safeConnections = Array.isArray(savedConnections)
    ? savedConnections.filter((c) => c && typeof c === 'object' && c.id)
    : [];
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm font-extrabold text-purple-950 uppercase tracking-wider">
              Thư Viện Tài Khoản Đang Quản Lý ({safeConnections.length})
            </h2>
          ...
          {safeConnections.length > 0 && (
            <>
              ...
              >
                ...
                Xuất File Sao Lưu Toàn Bộ ({safeConnections.length} TK)
              </button>
        ...
        {safeConnections.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 px-4 text-center space-y-3">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-slate-400 font-light max-w-sm leading-relaxed">
                Hãy dán session ChatGPT hoặc Claude ở trên và nhấn <strong className="text-purple-600">"Lưu Vào Danh Sách"</strong>, hoặc click <strong className="text-purple-655">"Nhập File Backup Cũ"</strong> để tải lên toàn bộ tài khoản cũ đã có.
              </p>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {safeConnections.map((conn) => {
              ...
              const isClaude = conn.provider === 'claude';
              const emailOrName = conn.email || conn.name || 'Tài khoản 9Router';
              const planTag = isClaude 
                ? 'CLAUDE WEB' 
                : (conn.providerSpecificData?.chatgptPlanType || 'PLUS').toUpperCase();
              ...
              return (
                ...
                >
                  ...
                  <button
                    onClick={() => handleDeleteConnection(conn.id, emailOrName)}
                    className="absolute top-3 right-3 text-slate-355 hover:text-rose-600 p-1.5 rounded-lg hover:bg-rose-50 transition-colors"
                  ...
                  <div className="flex items-start gap-2.5 pr-6">
                    ...
                    <div className="overflow-hidden min-w-0">
                      <h3 className="text-xs font-extrabold text-purple-950 truncate" title={emailOrName}>
                        {emailOrName}
                      </h3>
                      <p className="text-[9px] text-slate-400 font-mono mt-0.5 truncate select-all">
                        ID: {String(conn.id).substring(0, 8)}...
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className={`px-2 py-0.5 rounded text-[8px] font-bold tracking-wider uppercase flex items-center gap-1 ${
                      isClaude 
                        ? 'bg-amber-100 text-amber-800 border border-amber-200' 
                        : 'bg-purple-100 text-purple-700 border border-purple-200/40'
                    }`}>
                      {isClaude ? <Cpu className="w-2.5 h-2.5" /> : <Zap className="w-2.5 h-2.5" />}
                      {planTag}
                    </span>
